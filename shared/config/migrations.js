 ...
 *
 * @module shared/config/migrations
 ...
 */
const VALID_GROQ_VOICES = ['hannah', 'diana', 'autumn', 'troy', 'austin', 'daniel'];
 ...
 *
 * @type {Array<{version: number, key: string, description: string, migrate: Function}>}
 */
export const migrations = [
  {
     ...
     * @param {Object} stored - Current stored settings
     * @param {Function} save - Function to save settings
  ...
  },
  {
    ...
    migrate: async (stored, save) => {
      ...
      if (VALID_GROQ_VOICES.includes(currentVoice)) {
        return stored;
      }
 ...
 * @param {Object} stored - Current stored settings from browser.storage.local
 * @param {Function} save - Function to save settings (receives partial object)
 * @returns {Promise<Object>} Settings after all migrations applied
 */
export async function applyMigrations(stored, save) {
  let current = { ...stored };
 ...
 * @param {Object} stored - Current stored settings
 * @returns {number} Number of pending migrations
 */
export function getPendingMigrationCount(stored) {
  const currentVersion = stored._configVersion || 0;
